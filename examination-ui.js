...
function build(){
  ...
  var style=document.createElement('style');style.id='clean-exam-ui-style';style.textContent=`
    ...
    .clean-exam-decision.bad{background:#fff0f0;border:1px solid #e1aaaa}
  ...
  function updateGrade(){
    ...
    if(!Number.isFinite(n)||n<0||n>10){gres.style.display='none';set('examGrade','Δεν βαθμολογήθηκε');return}
    var label=n<5?'Ανεπιτυχής':n<6.5?'Καλώς — Επιτυχής':n<8.5?'Λίαν Καλώς — Επιτυχής':'Άριστα — Επιτυχής';
    gres.textContent=label;gres.style.display='block';set('examGrade',n.toFixed(2).replace('.',','));
    updateDecision(n);
  }
  function updateDecision(n){
    ...
    if(d==='Πλήρη'){
      dres.textContent='Η εξέταση του ΚΕΠ ολοκληρώθηκε με επιτυχία.';dres.classList.add('ok');dres.style.display='block';set('finalDecision','Επιτυχής');return
    }
    if(d==='Ελλιπή'){
      dres.textContent='Η εξέταση του ΚΕΠ ολοκληρώθηκε με επιτυχία, αλλά υπάρχει εκκρεμότητα στα δικαιολογητικά.';dres.classList.add('bad');dres.style.display='block';set('finalDecision','Επιτυχής — υπάρχει εκκρεμότητα στα δικαιολογητικά');return
    }
  ...
  gi.addEventListener('input',function(){var n=parseFloat(String(gi.value).replace(',','.'));if(Number.isFinite(n)&&n>=0&&n<=10)updateGrade();else{set('examGrade','Δεν βαθμολογήθηκε');dres.style.display='none';set('finalDecision','')}});
  p.__cleanExamSync=sync;p.__cleanExamGrade=gi;sync();
}
function syncVisibility(){
  var p=$('examinationPanel');if(!p)return;
  if(selectedKep()&&serviceEligible()){p.style.display='block';if(typeof p.__cleanExamSync==='function')p.__cleanExamSync();}
  else p.style.display='none';
}
function boot(){build();setTimeout(build,100);setTimeout(build,500);setTimeout(syncVisibility,150);setTimeout(syncVisibility,600);setTimeout(syncVisibility,1200);
  var r=$('result');if(r&&!r.__cleanExamObserver){var mo=new MutationObserver(function(){setTimeout(syncVisibility,0)});mo.observe(r,{childList:true,subtree:true,characterData:true});r.__cleanExamObserver=true}
  document.querySelectorAll('input[name="kep"]').forEach(function(x){if(!x.__cleanKepBound){x.__cleanKepBound=true;x.addEventListener('change',function(){setTimeout(syncVisibility,50);setTimeout(syncVisibility,300)})}})
}
