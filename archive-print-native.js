/* Native archive printing — wait for the popup document to finish layout before calling print(). */
(function(){
'use strict';
function install(frame){
  try{
    ...
    function nativeMakePdf(data,win){
      ...
      const dateText=v=>{const s=String(v||'');let x=null;if(/^\d{4}-\d{2}-\d{2}$/.test(s)){const [y,m,dd]=s.split('-').map(Number);x=new Date(y,m-1,dd,12)}else if(/^\d{2}\/\d{2}\/\d{4}$/.test(s)){const [dd,m,y]=s.split('/').map(Number);x=new Date(y,m-1,dd,12)}else{x=new Date(s)}return isNaN(x)?'Δεν έχει καταχωρηθεί':x.toLocaleDateString('el-GR',{weekday:'long',day:'numeric',month:'numeric',year:'numeric'})};
...
if(f){f.addEventListener('load',()=>setTimeout(()=>install(f),50));if(f.contentDocument&&f.contentDocument.readyState==='complete')setTimeout(()=>install(f),50)}
})();
