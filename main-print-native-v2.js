/* Native main print/PDF controller. No canvas and no dependency on lexical helper functions. */
(function(){
'use strict';
...
function report(){
 ...
 return `<!doctype html><html lang="el"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Αρχείο Εξέτασης ΚΕΠ — ${esc(registry)}</title><style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff;color:#222;font-family:Arial,sans-serif}.sheet{width:210mm;min-height:297mm;padding:13mm;display:flex;flex-direction:column;break-after:page;page-break-after:always}.head{text-align:center;border-bottom:2px solid #214f83;padding-bottom:5mm;margin-bottom:6mm}.brand{font-size:22pt;font-weight:800;color:#214f83}.title{font-size:16pt;font-weight:800;color:#214f83;margin-top:2mm}.sub{font-size:9pt;color:#777;margin-top:2mm}h2{font-size:11pt;color:#214f83;margin:5mm 0 2mm}table{width:100%;border-collapse:collapse;font-size:9.5pt}th,td{border:1px solid #cfd6df;padding:3mm;text-align:left;vertical-align:top}th{width:34%;background:#f0f3f7;font-weight:800}.result{border:1px solid #cfd6df;padding:4mm;font-size:10pt;line-height:1.45}.result button,.result input,.result select,.result textarea{display:none}.foot{margin-top:auto;border-top:1px solid #d5dbe2;padding-top:2mm;font-size:7.5pt;color:#777;display:flex;justify-content:space-between}</style></head><body><section class="sheet"><div class="head"><div class="brand">SeaService Calculator</div><div class="title">ΑΡΧΕΙΟ ΕΞΕΤΑΣΗΣ ΚΕΠ</div><div class="sub">Εκτύπωση / Αποθήκευση ως PDF — ${esc(now)}</div></div><h2>ΣΤΟΙΧΕΙΑ ΝΑΥΤΙΚΟΥ</h2><table>${rows.slice(0,4).map(r=>`<tr><th>${esc(r[0])}</th><td>${esc(r[1])}</td></tr>`).join('')}</table><h2>ΣΤΟΙΧΕΙΑ ΕΞΕΤΑΣΗΣ</h2><table>${rows.slice(4).map(r=>`<tr><th>${esc(r[0])}</th><td>${esc(r[1])}</td></tr>`).join('')}</table><h2>ΑΠΟΤΕΛΕΣΜΑ ΥΠΟΛΟΓΙΣΜΟΥ</h2><div class="result">${resultHtml}</div><div class="foot"><span>© 2026 SeaService Calculator. All rights reserved.</span><em>Σχεδιασμός &amp; Ανάπτυξη: Tassos Ballas</em></div></section></body></html>`;
}
