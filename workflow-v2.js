...
function refresh(){
 const rec=restoreArchivedService();
 const p=$('examinationPanel'),t=resultText();if(!p||!t)return;
...
function buildRecord(){
 ...
 if(docs!=='Εντάξει')return{id:old?.id||Date.now(),timestamp:z.iso,date:z.date,time:z.time,registryNumber:r,fullName:name,kep:k?'ΚΕΠ '+k:'',examiner:ex,documents:docs,documentsNote:note,examinationStatus:'Εκκρεμεί εξέταση',grade:'Δεν βαθμολογήθηκε',finalDecision:'Εκκρεμεί εξέταση',result:t+'\n\nΠαρατήρηση δικαιολογητικών: '+note,success:false,trips:mergedTrips,previousVisits:previous};
 ...
 return{id:old?.id||Date.now(),timestamp:z.iso,date:z.date,time:z.time,registryNumber:r,fullName:name,kep:k?'ΚΕΠ '+k:'',examiner:ex,documents:'Εντάξει',documentsNote:'',examinationStatus:done?'Εξετάστηκε':'Εκκρεμεί εξέταση',grade,finalDecision:decision,result:t,success,trips:mergedTrips,previousVisits:previous}
}
...
function init(){document.addEventListener('click',intercept,true);document.addEventListener('change',e=>{if(e.target?.id==='documentsStatus'||e.target?.id==='examinationStatus'||e.target?.name==='kep'||e.target?.id==='registryNumber'){lastKey='';refresh()}});document.addEventListener('input',e=>{if(e.target?.id==='examGradeNumber'||e.target?.id==='registryNumber')refresh()});const r=$('result');if(r)new MutationObserver(refresh).observe(r,{childList:true,subtree:true,characterData:true});setInterval(refresh,500);refresh()}
